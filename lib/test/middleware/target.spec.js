...
test('insyn:middleware:target - makeTargetMiddleware combines fetcher and diff middleware', t => {
    ...
    expect(result, 'to have key', 'targetDiff');
});
...
test('insyn:middleware:target - target is fetched and diffed afterwards', t => {
    const service = createService(_handler,
        applyMiddleware(makeTargetMiddleware(_fetcher))
    );
    const response = service.consume(action);
    expect(response, 'to have key', 'targetDiff');
    expect(response.targetDiff, 'to satisfy', {
        name: expect.it('to satisfy', {
            oldValue: 'Jack Sparrow',
            newValue: 'Jack_Sparrow',
        })
    })
});

test.todo('Diff also works without target (For Creation)');
